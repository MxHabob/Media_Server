...
#pragma warning restore CA1862
        ...
        internal async Task<User> CreateUserInternalAsync(string name, JellyfinDbContext dbContext, string? pin = null, SubscriptionType subscriptionType = SubscriptionType.None)
        {
        ...
        /// <summary>
        /// Creates multiple users with random PINs and specified subscription type.
        /// </summary>
        /// <param name="count">Number of PINs to generate.</param>
        /// <param name="subscriptionType">The subscription type for the users.</param>
        /// <returns>A list of generated PINs.</returns>
        public async Task<List<string>> GeneratePinsAsync(int count, SubscriptionType subscriptionType)
        {
            if (count <= 0)
            {
            ...
            await using (dbContext.ConfigureAwait(false))
            {
                ...
                {
                    var pin = GenerateSecurePin(10); // 10-digit secure PIN
                    var username = $"PIN_{pin}"; // Unique username based on PIN
                    if (Users.Any(u => u.Username.Equals(username, StringComparison.OrdinalIgnoreCase)))
                    {
                        i--; // Retry if username exists
                        continue;
                    }
            ...
            await _eventManager.PublishAsync(new GenericEventArgs<List<string>>(pins)).ConfigureAwait(false);
            return pins;
        }
        ...
        public async Task<User?> AuthenticateUserByPinAsync(string pin, string remoteEndPoint, bool isUserSession)
        {
            if (string.IsNullOrWhiteSpace(pin))
            {
                _logger.LogInformation("Authentication request without PIN has been denied (IP: {IP}).", remoteEndPoint);
            ...
            {
                _logger.LogInformation("Authentication request for {UserName} has been denied due to expired subscription (IP: {IP}).", user.Username, remoteEndPoint);
                throw new SecurityException("User subscription has expired.");
            }
        ...
        /// <summary>
        /// Calculates the expiration date based on subscription type.
        /// </summary>
        /// <param name="subscriptionType">The subscription type.</param>
        /// <returns>The expiration date or null for lifetime.</returns>
        private static DateTime? CalculateExpirationDate(SubscriptionType subscriptionType)
        {
        ...
        public UserDto GetUserDto(User user, string? remoteEndPoint = null)
        {
            ...
            return new UserDto
            {
                ...
                SubscriptionType = (int)user.SubscriptionType,
